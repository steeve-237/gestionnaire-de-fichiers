        ...
        if curent_lick_time - last_click_time > 0.5:
            ...
            if self.type=="dir": #sil est un dossier
                if self.foled=="yes": #s'il n'est pas encore deployé on le deploye
                    ...
                    try:
                        # on essaie de trouver un chemain qui mene au dossier courrent depuis le dossier actuel
                        ...
                        # donc on le rend invisible pour le rerendre visible a la prochaine ouverture
                        for elt in self.content.content.content.controls[1:]:
                            elt.visible=False
                    except: #sinon on veut juste le fermer donc on detruit tous ses enfants sauf le titre (le premier enfant)
...
def onchangeresearch(*args):
    ...
    for item in list_item2:
        if (researchfield.value in item.name):
